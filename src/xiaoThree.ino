...
static float printFloat(float val, bool valid) {
	if (!valid) {
    smartDelay(0);
		return 0.00;
	} else {
...
void loop() {
  ...
  myData.id=5;
  myData.b = printFloat(gps.location.lat(), gps.location.isValid());
  myData.c = printFloat(gps.location.lng(), gps.location.isValid());
